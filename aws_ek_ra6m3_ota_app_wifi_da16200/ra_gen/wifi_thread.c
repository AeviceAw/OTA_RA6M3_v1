...
sci_uart_instance_ctrl_t g_wifi0_ctrl;

baud_setting_t g_wifi0_baud_setting =
        {
        /* Baud rate calculated with 1.725% error. */.semr_baudrate_bits_b.abcse = 0,
          .semr_baudrate_bits_b.abcs = 0, .semr_baudrate_bits_b.bgdm = 1, .cks = 0, .brr = 15, .mddr = (uint8_t) 256, .semr_baudrate_bits_b.brme =
                  false };

/** UART extended configuration for UARTonSCI HAL driver */
const sci_uart_extended_cfg_t g_wifi0_cfg_extend =
{ .clock = SCI_UART_CLOCK_INT, .rx_edge_start = SCI_UART_START_BIT_FALLING_EDGE, .noise_cancel =
          SCI_UART_NOISE_CANCELLATION_DISABLE,
  .rx_fifo_trigger = SCI_UART_RX_FIFO_TRIGGER_MAX, .p_baud_setting = &g_wifi0_baud_setting, .flow_control =
...
const uart_cfg_t g_wifi0_cfg =
{ .channel = 0, .data_bits = UART_DATA_BITS_8, .parity = UART_PARITY_OFF, .stop_bits = UART_STOP_BITS_1, .p_callback =
          Da16200_uart_callback,
  .p_context = NULL, .p_extend = &g_wifi0_cfg_extend,
...
const uart_instance_t g_wifi0 =
{ .p_ctrl = &g_wifi0_ctrl, .p_cfg = &g_wifi0_cfg, .p_api = &g_uart_on_sci };
