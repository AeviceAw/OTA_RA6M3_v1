...
void ota_thread_create(void);
static void ota_thread_func(void *pvParameters);
void rtos_startup_err_callback(void *p_instance, void *p_data);
void rtos_startup_common_init(void);
extern uint32_t g_fsp_common_thread_count;

const rm_freertos_port_parameters_t ota_thread_parameters =
{ .p_context = (void*) NULL, };
...
void ota_thread_create(void)
{
    ...
    ota_thread = xTaskCreateStatic (
                    ...
                    BaseType_t ota_thread_create_err = xTaskCreate(
                                    ...
                                    3,
#if 1
                                    (StackType_t*) &ota_thread_stack,
                                    (StaticTask_t*) &ota_thread_memory
#else
                        & ota_thread
                        #endif
                                    );
    ...
    if (NULL == ota_thread)
    {
        rtos_startup_err_callback (ota_thread, 0);
    }
#else
                    if (pdPASS != ota_thread_create_err)
                    {
                        rtos_startup_err_callback(ota_thread, 0);
                    }
                    #endif
