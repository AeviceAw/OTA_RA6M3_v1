...
{ .channel = 1, .trigger = EXTERNAL_IRQ_TRIG_FALLING, .filter_enable = false, .clock_source_div =
          EXTERNAL_IRQ_CLOCK_SOURCE_DIV_64,
  .p_callback = external_irq1_callback,
...
#endif
void rtos_startup_err_callback(void *p_instance, void *p_data);
SemaphoreHandle_t g_data_semaphore;
#if 1
StaticSemaphore_t g_data_semaphore_memory;
#endif
void rtos_startup_err_callback(void *p_instance, void *p_data);
SemaphoreHandle_t g_wifi_semaphore;
#if 1
StaticSemaphore_t g_wifi_semaphore_memory;
#endif
void rtos_startup_err_callback(void *p_instance, void *p_data);
SemaphoreHandle_t g_rgb_semaphore;
#if 1
StaticSemaphore_t g_rgb_semaphore_memory;
#endif
void rtos_startup_err_callback(void *p_instance, void *p_data);
SemaphoreHandle_t g_publish_semaphore;
#if 1
...
void g_common_init(void)
{
    ...
    g_data_semaphore =
#if 1
            xSemaphoreCreateBinaryStatic (&g_data_semaphore_memory);
#else
                xSemaphoreCreateBinary();
                #endif
    if (NULL == g_data_semaphore)
    {
        rtos_startup_err_callback (g_data_semaphore, 0);
    }
    g_wifi_semaphore =
#if 1
            xSemaphoreCreateBinaryStatic (&g_wifi_semaphore_memory);
#else
                xSemaphoreCreateBinary();
                #endif
    if (NULL == g_wifi_semaphore)
    {
        rtos_startup_err_callback (g_wifi_semaphore, 0);
    }
    g_rgb_semaphore =
#if 1
            xSemaphoreCreateBinaryStatic (&g_rgb_semaphore_memory);
#else
                xSemaphoreCreateBinary();
                #endif
    if (NULL == g_rgb_semaphore)
    {
        rtos_startup_err_callback (g_rgb_semaphore, 0);
    }
    g_publish_semaphore =
#if 1
            xSemaphoreCreateBinaryStatic (&g_publish_semaphore_memory);
#else
                xSemaphoreCreateBinary();
                #endif
    if (NULL == g_publish_semaphore)
    {
