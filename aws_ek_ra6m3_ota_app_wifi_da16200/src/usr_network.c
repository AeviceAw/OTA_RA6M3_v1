...
static fsp_err_t wifi_preinit(void)
{
    ...
    strcpy (g_wifi.ssid, p_string);

    p_string = strtok(NULL, " ");
    strcpy (g_wifi.pwd, p_string);
#else
    strcpy (g_wifi.ssid, WIFI_SSID);
    strcpy (g_wifi.pwd, WIFI_PASSWORD);
#endif

    g_wifi.security = WIFI_SECURITY;
...
fsp_err_t network_interface_setup(void)
{
    ...
    g_wifi.device_ip_addr[0] = (uint8_t)(wifi_ipinfo.xIPAddress.ulAddress[0]);
    g_wifi.device_ip_addr[1] = (uint8_t)(wifi_ipinfo.xIPAddress.ulAddress[0] >> 8);
    g_wifi.device_ip_addr[2] = (uint8_t)(wifi_ipinfo.xIPAddress.ulAddress[0] >> 16);
    g_wifi.device_ip_addr[3] = (uint8_t)(wifi_ipinfo.xIPAddress.ulAddress[0] >> 24);
    ...
    g_wifi.device_ip_addr[0] = (uint8_t)wifi_ipinfo.xIPAddress.ulAddress[0];
    g_wifi.device_ip_addr[1] = (uint8_t)wifi_ipinfo.xIPAddress.ulAddress[1];
    g_wifi.device_ip_addr[2] = (uint8_t)wifi_ipinfo.xIPAddress.ulAddress[2];
    g_wifi.device_ip_addr[3] = (uint8_t)wifi_ipinfo.xIPAddress.ulAddress[3];

    NETWORK_SETUP_APP_PRINT(WIFI_POSTINIT_INFO);
    NETWORK_SETUP_APP_PRINT("\tDescription . . . . : Renesas "KIT_NAME" + Silex Wi-Fi\r\n");
    NETWORK_SETUP_APP_PRINT("\tIPv4 Address  . . . : %3d.%3d.%3d.%3d\r\n",
                            g_wifi.device_ip_addr[0], g_wifi.device_ip_addr[1], g_wifi.device_ip_addr[2], g_wifi.device_ip_addr[3]);
#endif
#endif
